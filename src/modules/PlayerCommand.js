...
const commands = [
    new Command("help", "lists available commands, or command usage", "[cmd]", (player, args) => {
        let cmd = args[1];
    ...
    new Command("id", "gets your playerID", "", (player, args) => {
        send(player, `Your PlayerID is ${player.pID}`);
    }),
    new Command("kill", "self kill", "", (player, args) => {
    ...
    new Command("skin", "change skin", "[skin]", (player, args) => {
        if (player.cells.length)
    ...
    new Command("login", "upgrade your credential to access more commands", "<password>", (player, args) => {
        let pass = args[1];
